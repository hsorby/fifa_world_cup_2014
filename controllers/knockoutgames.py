...
def _initialisePredictions():
    ...
    all_predictions = db(db.knockout_predictions.user_id==auth.user_id).select(db.knockout_predictions.match_number, groupby=db.knockout_predictions.match_number)
    ...
    if len(all_predictions) != len(all_matches):
        ...
        for match_number in all_match_numbers:
            if match_number not in all_user_match_numbers:
                db.knockout_predictions.insert(match_number=match_number, 
...
def index():
    ...
    left = [db.knockout_predictions.on(db.knockout_match.match_number == db.knockout_predictions.match_number), 
...
def predict_match():
    ...
    if request.post_vars:
        ...
        db.knockout_predictions.update_or_insert((db.knockout_predictions.match_number==knockout_match.match_number) &( db.knockout_predictions.user_id==auth.user_id),
                                    match_number=knockout_match.match_number,
                                    user_id=auth.user_id,
